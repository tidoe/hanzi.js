...
for(var i = 0; i < hanzi.length; ++i){
	...
	for(var s = 0; s < seq.length; ++s){
		...
		if(seq[s] == "⿰"){
			...
			var sx = ((x2-x1)/100.0);
			var sy = ((y2-y1)/100.0);
			x1 = Math.round(0.9*x1/sx).toString() + "%"; // 0.90 is a magic number
			y2 = Math.round(0.9*y2/sy).toString() + "%"; // --------- " ---------
		...
		var dfill = "fill-opacity='0'";
		if(seq.length == 1){ // single descriptors shall not be transparent
			dfill = "fill='" + colors[s] + "'";
		}
		svg += "<text x='0%' y='90%' font-size='" + (1.05*parseInt(fs)).toString() + ms + "' " + dfill + ">" + dsetValues[descriptors.indexOf(seq[s])] + "</text>";
	}
